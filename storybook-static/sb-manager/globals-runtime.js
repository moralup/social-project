...
function prepareForTelemetry(originalError) {
    ...
    return (
        (originalError.target === scope ||
            ...
            ((error = new Error(originalError.message)), (error.name = originalError.name || error.name)),
        originalError.fromStorybook || (error = new UncaughtManagerError({ error })),
...
scope.sendTelemetryError = (error) => {
    shouldSkipError(error) || scope.__STORYBOOK_ADDONS_CHANNEL__.emit(TELEMETRY_ERROR, prepareForTelemetry(error));
};
